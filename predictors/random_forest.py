...
Defines the function for hyperparameter search and deployment for Random Forest 
"""
from sklearn.ensemble import RandomForestRegressor
from sklearn.model_selection import GridSearchCV, GroupKFold, RandomizedSearchCV
from sklearn.preprocessing import StandardScaler

def search_rf_grid(X_train_val, y_train_val, groups, test_grid, feature_set):
    ...
    print(f"\n[RF — {feature_set}] Stage 1: Coarse GridSearch...")
    ...
    plot_gs_trends(gs.cv_results_, list(test_grid.keys()), feature_set, "RF")
...
def search_rf_rs(X_train_val, y_train_val, groups, N_iter, test_param, feature_set):
    """
    Stage 2: nested spatial GroupKFold CV with RandomizedSearchCV.

    Run `search_rf_grid` first and inspect its trend plots to choose a narrowed `test_param` range before calling this.
    ...
    A separate RandomizedSearchCV pass is run over the FULL train_val set specifically to select the final hyperparameters used downstream in `deploy_rf`. 
    """
    ...
    print(f"\n[RF — {feature_set}] Outer CV results:")
    ...
    plot_parallel_coords(rs_final.cv_results_, list(test_param.keys()), feature_set, "RF")
    plot_hp_importance(rs_final.cv_results_, list(test_param.keys()), feature_set, "RF")

    return cv_df, best_params


def deploy_rf(X_, y_, param_dist):
    """
    Refit Random Forest on the full given set (typically train_val) with chosen params.
    """
    scaler_final = StandardScaler()
    X_scaled = scaler_final.fit_transform(X_)
    final_model = RandomForestRegressor(**param_dist, random_state=42, n_jobs=-1)
